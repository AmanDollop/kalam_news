...
class ApiIntrigation{
  ...
  static Future<http.Response?> forgotPasswordApi({required Map<String, dynamic> bodyParams}) async {
    http.Response? response = await MyHttp.postMethod(
      url: '${ApiUrls.baseUrl}${ApiUrls.apiEndPointForgotPassword}',
      bodyParams: bodyParams,
      context: Get.context!,
    );
    if (response != null) {
  ...
  static Future<http.Response?> addBankDetailApi({required Map<String, dynamic> bodyParams, required String endPoint,File? image}) async {

    String token = await userToken(stringToken: true);

    http.Response? response = await MyHttp.multipartRequest(
        multipartRequestType: 'POST',
        userProfileImageKey: ApiConstantVar.bankPassbook,
